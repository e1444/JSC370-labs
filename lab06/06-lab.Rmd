...
ids <- stringr::str_remove_all(ids, "<Id>|</Id>") # Don't need this since we make use of the lookbehind and lookahead
```
...
abstracts <- str_extract(pub_char_list, "<Abstract>(?s).*?</Abstract>")
abstracts <- str_remove_all(abstracts, "<(?s).*?>")
abstracts <- str_remove_all(abstracts, "^\\s*|\\s*$")
```

  - How many of these don't have an abstract? 

```{r count-no-abstract, eval = FALSE}
sum(is.na(abstracts))
```
...
titles <- str_extract(pub_char_list, "<ArticleTitle>(?s).*?</ArticleTitle>")
titles <- str_remove_all(titles, "<(?s).*?>")
```

- How many of these don't have a title ? 

```{r count-no-title, eval = FALSE}
sum(is.na(titles))
```
